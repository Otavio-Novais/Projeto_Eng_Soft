...
import { ArrowRight, Plus, Grid, Clock, ChevronLeft, ChevronRight } from 'lucide-react';
    ...
    if (contextTrips.length > 0) {
      const mapped = contextTrips.map(t => {
        ...
        return {
          ...
          destino: t.destino,
          image: t.imagem ? (t.imagem.startsWith('http') ? t.imagem : `${API_BASE_URL}${t.imagem}`) : '/assets/images/default-trip.png',
        ...
        <div className="dashboard-main">
          ...
          {contextLoading ? (
            ...
            <div className="welcome-banner">
              ...
              <button onClick={() => setIsModalOpen(true)} className="btn-create-first">
                Criar Primeira Viagem <Plus size={18} />
              </button>
            </div>
          ) : (
            <div className="hero-carousel">
              <div className="hero-card" style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.5), rgba(0,0,0,0.7)), url(${recentTripsForCarousel[currentTripIndex].image})` }}>
                <div style={{ marginBottom: '1rem' }}>
                  <span className="hero-badge">
                    Viagem Recente ({currentTripIndex + 1}/{recentTripsForCarousel.length})
                  </span>
                </div>
                <h2 className="hero-title">{recentTripsForCarousel[currentTripIndex].title}</h2>
                ...
                <button onClick={() => navigate(`/trip/${recentTripsForCarousel[currentTripIndex].id}`)} className="btn-hero-action">
                  Ver Detalhes <ArrowRight size={18} />
                </button>
              </div>
              {recentTripsForCarousel.length > 1 && (
                <>
                  <button onClick={prevTrip} className="carousel-control prev"><ChevronLeft size={24} /></button>
                  <button onClick={nextTrip} className="carousel-control next"><ChevronRight size={24} /></button>
                </>
              )}
            </div>
          )}
          ...
          <div className="section-header">
            ...
            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
              <button onClick={() => navigate('/mytrips')} className="btn-view-all" style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#4b5563', fontWeight: '600', background: 'none', border: 'none', cursor: 'pointer' }}>
                Ver Minhas Viagens <ArrowRight size={18} />
              </button>
              <button onClick={() => setIsModalOpen(true)} className="btn-new-trip">
          ...
          {contextLoading ? (
            ...
            <div style={{ position: 'relative' }}>
              {mappedTrips.length > 3 && (
                <>
                  <button onClick={() => { const container = document.getElementById('trips-carousel-bottom'); if (container) container.scrollLeft -= 320; }} className="carousel-control prev" style={{ top: '50%', transform: 'translateY(-50%)', left: '-20px' }}>
                    <ChevronLeft size={24} />
                  </button>
                  <button onClick={() => { const container = document.getElementById('trips-carousel-bottom'); if (container) container.scrollLeft += 320; }} className="carousel-control next" style={{ top: '50%', transform: 'translateY(-50%)', right: '-20px' }}>
